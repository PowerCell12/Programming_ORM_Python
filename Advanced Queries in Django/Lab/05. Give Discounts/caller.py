...
def product_quantity_ordered():
    products = Product.objects.annotate(total_ordered_quantity=Sum('orderproduct__quantity')).order_by('-total_ordered_quantity').exclude(total_ordered_quantity=None)

    list1 = []

    for thing in products:
        list1.append(f"Quantity ordered of {thing.name}: {thing.total_ordered_quantity}")

    return "\n".join(list1)


def ordered_products_per_customer():
    fetched = Order.objects.prefetch_related('orderproduct_set__product__category').order_by('id')
...
def filter_products():
    query = Q(price__gt=3.00) & Q(is_available=True)  ## check
    products = Product.objects.all().filter(query).order_by('-price', 'name')
    list1 = []

    for thing in products:
        list1.append(f"{thing.name}: {thing.price}lv.")
...
def give_discount():
    ...
    for product in Product.objects.available_products().order_by('-price', 'name'):
        list1.append(f"{product.name}: {product.price}lv.")
