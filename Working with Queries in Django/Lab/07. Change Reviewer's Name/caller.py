...
def find_books_by_genre_and_language(genre_final, book_language_final):
    books = Book.objects.filter(genre=genre_final, language=book_language_final)
    return books
...
def find_authors_nationalities():
    ...
    for author in authors:
        list1.append(f"{author.first_name} {author.last_name} is {author.nationality}")
...
def  order_books_by_year():
    ...
    for book in books:
        list1.append(f"{book.publication_year} year: {book.title} by {book.author}")
...
def filter_authors_by_nationalities(nationality1):
    authors = Author.objects.filter(nationality=nationality1).order_by('first_name', 'last_name')
    list1 = []

    for author in authors:
        if author.biography is None:
            list1.append(f"{author.first_name} {author.last_name}")
        else:
            list1.append(author.biography)
...
def filter_authors_by_birth_year(first_year, second_year):
    ...
    for author in authors:
        list1.append(f"{author.birth_date}: {author.first_name} {author.last_name}")

    return '\n'.join(list1)


def change_reviewer_name(name1,  new_name1):
    Review.objects.filter(reviewer_name=name1).update(reviewer_name=new_name1)
